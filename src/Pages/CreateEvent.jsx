...
const handleImageRemove = (e, index) => {
  ...
  //* Location input suggestion state
 ...
 const [loading , setloading ] = useState(false);
  ...
  return (
    <form className='bg-[#E1E1E1]  h-screen w-full wrapper' onSubmit={handleSubmit}>
      <Navbar />
      {!loading && (<main className='px-10 py-4 mx-32 my-8 bg-white rounded-lg min-h-rest'>
        <h1 className='flex justify-center text-3xl text-blue-950 font-semibold'>Create Your Event !</h1>
        ...
        />
          ...
          <select value={selectedCategory} onChange={handleCategoryChange} required className="block appearance-none w-full cursor-pointer bg-white border border-[#bdbdbd] text-gray-600 py-2 px-2 pr-8 rounded-lg shadow-sm leading-tight focus:outline-none focus:text-black focus:bg-white focus:border-gray-500">
            <option value="selected"  className=' disabled hidden'>Select a Category</option>
        ...
        <button onClick={() => setloading(true)} className='w-32 p-1 mt-4 text-white transition duration-300 ease-in-out  bg-blue-500 rounded-lg hover:scale-[1.01]'>Create</button>
        </div>
      </main>)}
      {loading && <Loading/>}
    </form>
